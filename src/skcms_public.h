...
//                                  D + E|encoded|^C
GFXCMS_API bool skcms_TransferFunction_makePQish(
        skcms_TransferFunction*, float A, float B, float C, float D, float E, float F);
// HLGish:
//            { K * sign(encoded) * ( (R|encoded|)^G )          when 0   <= |encoded| <= 1/R
//   linear = { K * sign(encoded) * ( e^(a(|encoded|-c)) + b )  when 1/R <  |encoded|
GFXCMS_API bool skcms_TransferFunction_makeScaledHLGish(
        skcms_TransferFunction*, float K, float R, float G, float a, float b, float c);

// Compatibility shim with K=1 for old callers.
static inline bool skcms_TransferFunction_makeHLGish(
        skcms_TransferFunction* fn, float R, float G, float a, float b, float c) {
...
static inline bool skcms_TransferFunction_makePQ(skcms_TransferFunction* tf) {
    return skcms_TransferFunction_makePQish(
            tf, -107 / 128.0f, 1.0f, 32 / 2523.0f, 2413 / 128.0f, -2392 / 128.0f, 8192 / 1305.0f);
}
// HLG mapping encoded [0,1] to linear [0,12].
static inline bool skcms_TransferFunction_makeHLG(skcms_TransferFunction* tf) {
    return skcms_TransferFunction_makeHLGish(
            tf, 2.0f, 2.0f, 1 / 0.17883277f, 0.28466892f, 0.55991073f);
}
...
GFXCMS_API bool skcms_ParseWithA2BPriority(
        const void*, size_t, const int priority[], int priorities, skcms_ICCProfile*);
...
static inline bool skcms_Parse(const void* buf, size_t len, skcms_ICCProfile* profile) {
    ...
    return skcms_ParseWithA2BPriority(
            buf, len, priority, sizeof(priority) / sizeof(*priority), profile);
}
...
GFXCMS_API int skcms_GetInputChannelCount(const skcms_ICCProfile*);
...
GFXCMS_API bool skcms_AdaptToXYZD50(float wx, float wy, skcms_Matrix3x3* toXYZD50);

// Returns a matrix to convert RGB color into XYZ adapted to D50, given the
// primaries and whitepoint of the RGB model.
GFXCMS_API bool skcms_PrimariesToXYZD50(float rx,
                                        float ry,
                                        float gx,
                                        float gy,
                                        float bx,
