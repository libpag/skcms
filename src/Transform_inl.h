...
static constexpr F F0 = F() + 0.0f, F1 = F() + 1.0f,
                   FInfBits = F() + 0x7f800000;  // equals 2139095040, the bit pattern of +Inf
#else
static constexpr F F0 = 0.0f, F1 = 1.0f,
...
template <typename T, typename P> SI T load(const P* ptr) {
    T val;
    memcpy(&val, ptr, sizeof(val));
    return val;
}
template <typename T, typename P> SI void store(P* ptr, const T& val) {
    memcpy(ptr, &val, sizeof(val));
}

// (T)v is a cast when N == 1 and a bit-pun when N>1,
// so we use cast<T>(v) to actually cast or bit_pun<T>(v) to bit-pun.
template <typename D, typename S> SI D cast(const S& v) {
#if N == 1
    return (D)v;
...
template <typename D, typename S> SI D bit_pun(const S& v) {
...
#else
template <typename C, typename T> SI T if_then_else(C cond, T t, T e) {
    return bit_pun<T>((cond & bit_pun<C>(t)) | (~cond & bit_pun<C>(e)));
}
#endif
...
Half_from_F(F f) {
    ...
    U32 sem = bit_pun<U32>(f), s = sem & 0x80000000, em = sem ^ s;

    // For simplicity we flush denorm half floats (including all denorm floats) to zero.
    return cast<U16>(
            if_then_else(em < 0x38800000, (U32)F0, (s >> 16) + (em >> 13) - ((127 - 15) << 10)));
#endif
}

// Swap high and low bytes of 16-bit lanes, converting between big-endian and little-endian.
#if defined(USING_NEON)
SI U16 swap_endian_16(U16 v) { return (U16)vrev16_u8((uint8x8_t)v); }
#endif

SI U64 swap_endian_16x4(const U64& rgba) {
    return (rgba & 0x00ff00ff00ff00ff) << 8 | (rgba & 0xff00ff00ff00ff00) >> 8;
}
...
SI F approx_pow(F x, float y) {
    return if_then_else((x == F0) | (x == F1), x, approx_exp2(approx_log2(x) * y));
}
...
SI F apply_sign(F x, U32 sign) { return bit_pun<F>(sign | bit_pun<U32>(x)); }
...
SI F apply_tf(const skcms_TransferFunction* tf, F x) {
    ...
    F v = if_then_else(x < tf->d, tf->c * x + tf->f, approx_pow(tf->a * x + tf->b, tf->g) + tf->e);
...
SI F apply_pq(const skcms_TransferFunction* tf, F x) {
    ...
    F v = approx_pow(
            max_(tf->a + tf->b * approx_pow(x, tf->c), F0) / (tf->d + tf->e * approx_pow(x, tf->c)),
            tf->f);
...
template <typename T, typename P> SI T load_3(const P* p) {
#if N == 1
    return (T)p[0];
#elif N == 4
...
template <typename T, typename P> SI T load_4(const P* p) {
#if N == 1
    return (T)p[0];
#elif N == 4
...
template <typename T, typename P> SI void store_3(P* p, const T& v) {
#if N == 1
    p[0] = v;
#elif N == 4
    p[0] = v[0];
...
template <typename T, typename P> SI void store_4(P* p, const T& v) {
#if N == 1
    p[0] = v;
#elif N == 4
    p[0] = v[0];
...
#elif N == 8
    U8 v = {p[ix[0]], p[ix[1]], p[ix[2]], p[ix[3]], p[ix[4]], p[ix[5]], p[ix[6]], p[ix[7]]};
#elif N == 16
    U8 v = {p[ix[0]],
            p[ix[1]],
            p[ix[2]],
...
SI U16 gather_16(const uint8_t* p, I32 ix) {
    ...
    auto load_16 = [p](int i) { return load<uint16_t>(p + 2 * i); };
#if N == 1
    U16 v = load_16(ix);
#elif N == 4
...
SI U32 gather_32(const uint8_t* p, I32 ix) {
    ...
    auto load_32 = [p](int i) { return load<uint32_t>(p + 4 * i); };
#if N == 1
    U32 v = load_32(ix);
#elif N == 4
...
SI U32 gather_24(const uint8_t* p, I32 ix) {
    ...
    auto load_24_32 = [p](int i) { return load<uint32_t>(p + 3 * i); };
...
SI void gather_48(const uint8_t* p, I32 ix, U64* v) {
    ...
    *v = U64{
    ...
    Half_I64 zero = {0, 0, 0, 0}, mask = {-1, -1, -1, -1};
...
SI F F_from_U8(U8 v) { return cast<F>(v) * (1 / 255.0f); }
...
SI F minus_1_ulp(F v) { return bit_pun<F>(bit_pun<U32>(v) - 1); }
...
SI F table(const skcms_Curve* curve, F v) {
    ...
    I32 lo = cast<I32>(ix), hi = cast<I32>(minus_1_ulp(ix + 1.0f));
...
static void clut(uint32_t input_channels,
    ...
    assert(0 < dim && dim <= 4);
    assert(output_channels == 3 || output_channels == 4);
    ...
    *r = *g = *b = F0;
    ...
    for (int combo = 0; combo < (1 << dim); combo++) {  // This loop can be done in any order.
        ...
        switch ((dim - 1) & 3) {  // This lets the compiler know there are no other cases to handle.
            case 3:
                ix += index[3 + (combo & 8) / 2];
                w *= weight[3 + (combo & 8) / 2];
                SKCMS_FALLTHROUGH;
                // fall through

            case 2:
                ix += index[2 + (combo & 4) * 1];
                w *= weight[2 + (combo & 4) * 1];
                SKCMS_FALLTHROUGH;
                // fall through

            case 1:
                ix += index[1 + (combo & 2) * 2];
        ...
        if (output_channels == 3) {
            if (grid_8) {
                sample_clut_8(grid_8, ix, &R, &G, &B);
            } else {
                sample_clut_16(grid_16, ix, &R, &G, &B);
            }
        } else {
            if (grid_8) {
                sample_clut_8(grid_8, ix, &R, &G, &B, &A);
            } else {
                sample_clut_16(grid_16, ix, &R, &G, &B, &A);
            }
        }
        *r += w * R;
        *g += w * G;
...
#define STAGE_PARAMS(MAYBE_REF)                                                 \
    SKCMS_MAYBE_UNUSED const char *src, SKCMS_MAYBE_UNUSED char *dst,           \
            SKCMS_MAYBE_UNUSED F MAYBE_REF r, SKCMS_MAYBE_UNUSED F MAYBE_REF g, \
            SKCMS_MAYBE_UNUSED F MAYBE_REF b, SKCMS_MAYBE_UNUSED F MAYBE_REF a, \
...
#define STAGE(name, arg) \
    DECLARE_STAGE(       \
            name, arg, [[clang::musttail]] return (*list.fn)(list, ctx, src, dst, r, g, b, a, i))
...
STAGE(load_a8, NoCtx) { a = F_from_U8(load<U8>(src + 1 * i)); }

STAGE(load_g8, NoCtx) { r = g = b = F_from_U8(load<U8>(src + 1 * i)); }
...
#else
    U32 R = load_3<U32>(rgb + 0), G = load_3<U32>(rgb + 1), B = load_3<U32>(rgb + 2);
...
#if defined(USING_NEON)
    uint16x4x3_t v = vld3_u16(rgb);
    U16 R = (U16)v.val[0], G = (U16)v.val[1], B = (U16)v.val[2];
#else
    U16 R = load_3<U16>(rgb + 0), G = load_3<U16>(rgb + 1), B = load_3<U16>(rgb + 2);
#endif
    r = F_from_Half(R);
    g = F_from_Half(G);
    b = F_from_Half(B);
...
#if defined(USING_NEON)
    uint16x4x4_t v = vld4_u16(rgba);
    U16 R = (U16)v.val[0], G = (U16)v.val[1], B = (U16)v.val[2], A = (U16)v.val[3];
#else
    U64 px = load<U64>(rgba);
    U16 R = cast<U16>((px >> 0) & 0xffff), G = cast<U16>((px >> 16) & 0xffff),
        B = cast<U16>((px >> 32) & 0xffff), A = cast<U16>((px >> 48) & 0xffff);
#endif
    r = F_from_Half(R);
    g = F_from_Half(G);
    b = F_from_Half(B);
...
STAGE(matrix_3x3, const skcms_Matrix3x3* matrix) {
    ...
    F R = m[0] * r + m[1] * g + m[2] * b, G = m[3] * r + m[4] * g + m[5] * b,
      B = m[6] * r + m[7] * g + m[8] * b;
...
STAGE(matrix_3x4, const skcms_Matrix3x4* matrix) {
    ...
    F R = m[0] * r + m[1] * g + m[2] * b + m[3], G = m[4] * r + m[5] * g + m[6] * b + m[7],
...
STAGE(lab_to_xyz, NoCtx) {
    ...
    F L = r * 100.0f, A = g * 255.0f - 128.0f, B = b * 255.0f - 128.0f;

    // Convert to CIE XYZ.
    F Y = (L + 16.0f) * (1 / 116.0f), X = Y + A * (1 / 500.0f), Z = Y - B * (1 / 200.0f);
...
STAGE(xyz_to_lab, NoCtx) {
    F X = r * (1 / 0.9642f), Y = g, Z = b * (1 / 0.8249f);
    ...
    F L = Y * 116.0f - 16.0f, A = (X - Y) * 500.0f, B = (Y - Z) * 200.0f;
...
STAGE(clut_B2A, const skcms_B2A* b2a) { clut(b2a, &r, &g, &b, &a); }

// From here on down, the store_ ops are all "final stages," terminating processing of this group.

FINAL_STAGE(store_a8, NoCtx) { store(dst + 1 * i, cast<U8>(to_fixed(a * 255))); }
...
FINAL_STAGE(store_ga88, NoCtx) {
    // g should be holding luminance (Y) (r,g,b ~~~> X,Y,Z)
    store<U16>(dst + 2 * i, cast<U16>(to_fixed(g * 255) << 0) | cast<U16>(to_fixed(a * 255) << 8));
}

FINAL_STAGE(store_4444, NoCtx) {
    store<U16>(dst + 2 * i,
               cast<U16>(to_fixed(r * 15) << 12) | cast<U16>(to_fixed(g * 15) << 8) |
                       cast<U16>(to_fixed(b * 15) << 4) | cast<U16>(to_fixed(a * 15) << 0));
}

FINAL_STAGE(store_565, NoCtx) {
    store<U16>(dst + 2 * i,
               cast<U16>(to_fixed(r * 31) << 0) | cast<U16>(to_fixed(g * 63) << 5) |
...
FINAL_STAGE(store_888, NoCtx) {
    ...
    U16 R = cast<U16>(to_fixed(r * 255)), G = cast<U16>(to_fixed(g * 255)),
...
FINAL_STAGE(store_8888, NoCtx) {
    store(dst + 4 * i,
          cast<U32>(to_fixed(r * 255)) << 0 | cast<U32>(to_fixed(g * 255)) << 8 |
                  cast<U32>(to_fixed(b * 255)) << 16 | cast<U32>(to_fixed(a * 255)) << 24);
}
...
FINAL_STAGE(store_101010x_XR, NoCtx) {
    ...
    static constexpr float range = max - min;
    store(dst + 4 * i,
          cast<U32>(to_fixed(((r - min) / range) * 1023)) << 0 |
...
FINAL_STAGE(store_1010102, NoCtx) {
    store(dst + 4 * i,
          cast<U32>(to_fixed(r * 1023)) << 0 | cast<U32>(to_fixed(g * 1023)) << 10 |
                  cast<U32>(to_fixed(b * 1023)) << 20 | cast<U32>(to_fixed(a * 3)) << 30);
}
...
FINAL_STAGE(store_16161616LE, NoCtx) {
    ...
    vst4_u16(rgba, v);
#else
    U64 px = cast<U64>(to_fixed(r * 65535)) << 0 | cast<U64>(to_fixed(g * 65535)) << 16 |
             cast<U64>(to_fixed(b * 65535)) << 32 | cast<U64>(to_fixed(a * 65535)) << 48;
    store(rgba, px);
#endif
}
...
FINAL_STAGE(store_161616BE, NoCtx) {
    ...
    vst3_u16(rgb, v);
#else
    U32 R = to_fixed(r * 65535), G = to_fixed(g * 65535), B = to_fixed(b * 65535);
...
FINAL_STAGE(store_hhh, NoCtx) {
    ...
    U16 R = Half_from_F(r), G = Half_from_F(g), B = Half_from_F(b);
#if defined(USING_NEON)
    uint16x4x3_t v = {{
            (uint16x4_t)R,
            (uint16x4_t)G,
...
FINAL_STAGE(store_hhhh, NoCtx) {
    ...
    U16 R = Half_from_F(r), G = Half_from_F(g), B = Half_from_F(b), A = Half_from_F(a);
#if defined(USING_NEON)
    uint16x4x4_t v = {{
            (uint16x4_t)R,
            (uint16x4_t)G,
    ...
    vst4_u16(rgba, v);
#else
    store(rgba, cast<U64>(R) << 0 | cast<U64>(G) << 16 | cast<U64>(B) << 32 | cast<U64>(A) << 48);
#endif
}
...
#define M(name)                                            \
    case Op::name:                                         \
        Exec_##name(*contexts++, src, dst, r, g, b, a, i); \
        break;
            SKCMS_WORK_OPS(M)
#undef M
#define M(name)                                            \
...
#define M(name) &Exec_##name,
            SKCMS_WORK_OPS(M) SKCMS_STORE_OPS(M)
#undef M
    };
